...
function applyThemeMode(themeMode) {
  ...
  const theme = safeTheme(themeMode);

  document.documentElement.dataset.theme = theme;
  document.documentElement.classList.toggle("dark", theme === "dark");

  try {
    localStorage.setItem("adf_theme_mode", theme);
    window.dispatchEvent(
      new CustomEvent("adf-theme-updated", {
        detail: { themeMode: theme },
      })
    );
  } catch {
    // ignore
...
function readDevProfile() {
  try {
    const stored = JSON.parse(
      localStorage.getItem(DEV_PROFILE_KEY) || "null"
    );

    return stored || defaultDevProfile();
  } catch {
...
function readCachedProfile() {
  try {
    const cached = JSON.parse(
      localStorage.getItem("adf_user_profile_cache") || "null"
    );

    if (cached?.theme_mode) {
      applyThemeMode(cached.theme_mode);
    }
...
export default function useAuthProfile() {
  ...
  function setDevSession(nextProfile) {
    const normalizedProfile = {
      ...defaultDevProfile(nextProfile),
      ...nextProfile,
      team: safeTeam(nextProfile?.team),
      theme_mode: safeTheme(nextProfile?.theme_mode),
      updated_at: new Date().toISOString(),
    };

    const nextUser = devUserFromProfile(normalizedProfile);

    localStorage.setItem(DEV_SESSION_KEY, "true");
    saveDevProfile(normalizedProfile);
  ...
  async function loadProfile(nextUser) {
    ...
    if (data) {
      const normalized = {
        ...data,
        team: safeTeam(data.team),
        avatar_key: data.avatar_key || DEFAULT_AVATAR_KEY,
        theme_mode: safeTheme(data.theme_mode),
      };
    ...
    const normalizedInserted = {
      ...inserted,
      team: safeTeam(inserted.team),
      avatar_key: inserted.avatar_key || DEFAULT_AVATAR_KEY,
      theme_mode: safeTheme(inserted.theme_mode),
    };

    setProfile(normalizedInserted);
    cacheProfile(normalizedInserted);
    setProfileLoading(false);
  ...
  async function updateProfile(updates = {}) {
    ...
    if (isDevSession) {
      ...
      if ("phone" in updates) {
        nextProfile.phone = cleanPhone(updates.phone);
      }
      ...
      if ("team" in updates) {
        nextProfile.team = safeTeam(updates.team);
      }
    ...
    if ("phone" in updates) {
      payload.phone = cleanPhone(updates.phone);
    }

    if ("display_name" in updates) {
      payload.display_name = clean(updates.display_name);
    }

    if ("team" in updates) {
      payload.team = safeTeam(updates.team);
    }
    ...
    if ("theme_mode" in updates) {
      payload.theme_mode = safeTheme(updates.theme_mode);
    }
  ...
  function updateThemeMode(themeMode) {
    const nextTheme = safeTheme(themeMode);
