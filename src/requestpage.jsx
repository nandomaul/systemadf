  ...
  const filteredRequests = useMemo(() => {
    ...
    let result = requests.filter((request) => {
      ...
      const matchKeyword =
        ...
        normalizeText(request.requester).includes(keyword) ||
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortBy === "requester") return String(a.requester).localeCompare(String(b.requester));
        ...
        <main className="flex h-screen min-w-0 flex-1 flex-col overflow-hidden px-4 py-5 md:px-8 lg:px-6">
          ...
          <div className="request-scroll min-h-0 flex-1 overflow-x-auto overflow-y-auto pr-0 lg:pr-2">
            <div className="min-w-[1180px]">
              ...
              {activeMenu === "notes" && <NotesPanel notes={notesData} />}
            </div>
          </div>
        </main>
      </div>
...
function CategoryCard({ category, active, onClick, index }) {
  return (
    <button
      onClick={onClick}
      className={`request-fade min-h-[160px] rounded-[28px] border p-5 text-left transition duration-500 active:scale-[0.985] ${
        active
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div
          className={`grid h-12 w-12 place-items-center rounded-2xl border text-lg font-semibold ${
            active ? "border-white/10 bg-white text-neutral-950" : "border-black/5 bg-white"
          }`}
        >
          {category.avatar}
        </div>
      ...
      <h3 className="mt-7 text-xl font-semibold tracking-[-0.05em]">
        {category.label}
      </h3>
      <p className={`mt-1 text-sm leading-5 ${active ? "text-white/60" : "text-neutral-500"}`}>
        {category.desc}
      </p>
        ...
        <div className="flex min-w-0 gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-5 text-neutral-500">
              {request.requester} · {request.type} · {request.platform || "Internal"}
            </p>
...
function CreateRequestPanel({
  ...
  return (
    <div className="lg:sticky lg:top-0 lg:h-[calc(100vh-238px)]">
      <div className="flex h-full flex-col overflow-hidden rounded-[30px] border border-white/70 bg-white/68 shadow-[0_18px_60px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        ...
        <div className="shrink-0 border-t border-black/5 p-5">
          <button
            ...
            disabled={saving}
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-neutral-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-60 active:scale-[0.985]"
          >
                ...
                >
                  {date && (
                    <>
                      ...
                      {isToday && (
                        <span className="absolute right-2 top-2 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-700">
                          Today
                        </span>
                      )}
                            ...
                            {dayNotes.slice(0, 5).map((note, dotIndex) => {
                              ...
                              return (
                                <span
                                  ...
                                  style={{ background: isSelected ? "#fff" : color.dot }}
                                />
                              );
                            })}
                          </div>
                          <p className={`truncate text-[10px] ${isSelected ? "text-white/70" : "text-neutral-400"}`}>
              ...
              {noteList.map((note, index) => {
                ...
                return (
                  <button
                    ...
                    className={`request-fade group relative overflow-hidden rounded-[24px] border p-4 text-left shadow-sm transition hover:bg-white ${
                      isActive ? "border-neutral-950 bg-white" : "border-white/70 bg-white/70"
