...
export default function useAuthProfile() {
  ...
  async function sendPasswordRecoveryCode(email) {
    ...
    if (isDevSession) {
      const message = "Password recovery is disabled in Developer Mode.";
      setAuthError(message);
      return { error: { message } };
    }
    ...
    const { data, error } = await supabase.auth.resetPasswordForEmail(nextEmail);
  ...
  async function verifyPasswordRecoveryCode({ email, token }) {
    setAuthError("");

    const nextEmail = cleanEmail(email);
    const cleanToken = clean(token).replace(/\D/g, "").slice(0, 6);

    if (!nextEmail || cleanToken.length !== 6) {
      const message = "Please enter the 6 digit recovery code.";
      setAuthError(message);
      return { error: { message } };
    }
